...
async function freeTrade(username: string, amount: number) {
    ...
    async function intervalfuncft() {
        ...
        for (let i = 0; i < times;) {
            ...
            try {
                tradeRef = await madfutClient.inviteUser(traderName,"testing12");
...
async function freeTradeUnlimited(username: string, coins: number, packs: Max3Array<{ pack: string, amount: number }>) {
    while (true) {
        ...
        let tradeRef;
        try {
            tradeRef = await madfutClient.inviteUser(username,"testing12");
...
bot.on("link", async (interaction, username) => {
    ...
    try {
        ...
        const trade = await madfutClient.inviteUser(traderName, "testing12");
